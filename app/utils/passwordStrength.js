export const getPasswordStrengthInTime = (password) => {
  ...
  const convertSecondsToTime = (seconds) => {
    ...
    if (Math.floor(seconds) < 1) {
      return "instantly hacked";
    } else if (seconds < 60) {
      return `${Math.floor(seconds)} second${seconds > 2 ? "s" : ""} to hack`;
    } else if (minutes < 60) {
      return `${Math.floor(minutes)} minute${minutes > 2 ? "s" : ""} to hack`;
    } else if (hours < 24) {
      return `${Math.floor(hours)} hour${hours > 2 ? "s" : ""} to hack`;
    } else if (days < 30) {
      return `${Math.floor(days)} day${days > 2 ? "s" : ""} to hack`;
    } else if (months < 12) {
      return `${Math.floor(months)} month${months > 2 ? "s" : ""} to hack`;
    } else if (years < thousand) {
      return `${Math.floor(years)} year${years > 2 ? "s" : ""} to hack`;
    } else if (years < oneMillion) {
      return `${Math.floor(years / thousand)} thousand years to hack`;
    } else if (years < oneBillion) {
      return `${Math.floor(years / oneMillion)} million years to hack`;
    } else if (years < oneTrillion) {
      return `${Math.floor(years / oneBillion)} billion years to hack`;
    } else {
      return `${Math.floor(years / oneTrillion)} trillion years to hack`;
    }
  };
  ...
  */
  ...
  const timePerHackingAllPossibilities =
    Math.pow(2, entropy) * timePerCheckingOnePassword; // s
  const timePerHackingPassword = timePerHackingAllPossibilities / 2; // s

  return `${password.length} chars - ${convertSecondsToTime(
    timePerHackingPassword,
  )}`;
};
